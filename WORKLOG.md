...
summary: "Reply-to endret fra andreas@aharstad.no (midlertidig siden Resend-oppsettet) til post@bimverdi.no — besluttet av Andreas i dag. MERK: konstanten er GLOBAL for all Resend-e-post (verifisering, invitasjoner, nyhetsbrev), ikke bare nyhetsbrevet — svar fra medlemmer på enhver systemmail går nå til post@bimverdi.no. Deployet til prod. DERETTER: massesend-plan skrevet og review-herdet — docs/plans/2026-06-10-001-feat-nyhetsbrev-massesend-motor-plan.md. Research (Resend-docs verifisert): batch-API 100/kall, rate 5 req/s per KALL, idempotency-keys 24t på begge endepunkter, List-Unsubscribe per element. Adversarial+feasibility-review (2 agenter) fant 16 funn, 2 KRITISKE: (1) idempotency-nøkler utløper etter 24t → dag-2-gjenopptak av usikker batch ville gitt 50 duplikater → 24-timers-vakt designet inn; (2) snapshot-regenerering var ulåst under kjøring → 409-vranglås + splittet innhold → frys HTML-hash+subject i manifest + redigeringssperre. Øvrige: atomisk kjøre-lås m/ heartbeat (504→Fortsett-race), persist-før-send ved chunk-ombygging, fail-closed miljøgate (konstant OG home_url===prod — blocklist-polaritet feiler åpent under WP-CLI/LAN/staging), dekobling av massesend-konstant fra send_en-allowlist (ville permanent kortsluttet test-vernet), ID-telling mot stille under-sending, karantene for Resend-avviste adresser. Andreas la til eksplisitt SIKKERHETSKONTRAKT i planen: INGEN e-post til reelle brukere under hele bygge-/testfasen — konstanten settes ALDRI lokalt, lokal testing kun mot gate-avvisning + stubbet API (pre_http_request), første ekte batch-kall = Unit 5-prosedyren på prod mot allowlist. Implementering starter etter Bård-møtet i morgen (xhigh-økt)."
status: done
