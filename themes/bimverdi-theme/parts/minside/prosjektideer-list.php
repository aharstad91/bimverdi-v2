 ...
 * Note: Project ideas cannot be edited after submission (per PRD).
 *
...
// Query user's project ideas (CPT is 'case')
$ideas = get_posts([
    'post_type'      => 'case',
    'posts_per_page' => -1,
    'author'         => $user_id,
...
<?php get_template_part('parts/components/page-header', null, [
    ...
    'actions' => [
        ['text' => __('Send inn ny idé', 'bimverdi'), 'url' => bimverdi_minside_url('prosjektideer/ny'), 'variant' => 'primary'],
    ],
]); ?>
    ...
    <div class="flex items-start gap-3">
        ...
        <div>
            <p class="text-sm text-[#5A5A5A]">
                <?php _e('Prosjektidéer er kun synlige for deg og BIM Verdi-administrasjonen. Godkjente idéer kan bli til pilotprosjekter. Idéer kan ikke redigeres etter innsending.', 'bimverdi'); ?>
            </p>
        </div>
    </div>
</div>
...
<?php if (empty($ideas)): ?>
    ...
    <?php get_template_part('parts/components/empty-state', null, [
        ...
        'cta_url' => bimverdi_minside_url('prosjektideer/ny'),
    ]); ?>
        ...
        <?php foreach ($ideas as $idea): ?>
            ...
            <div class="bg-white rounded-lg border border-[#E5E0D8] p-5">
                <div class="flex items-start justify-between gap-4">
                    ...
                    <!-- No edit button - project ideas cannot be edited after submission -->
                </div>
            </div>
        <?php endforeach; ?>
    </div>
